...
const setAnimations = (gltf: GLTF) => {
  ...
  if (gltf.animations) {
    // Automatically loop the newly created Mixamo animation!
    if (gltf.animations && gltf.animations.length > 0) {
      const defaultClip = gltf.animations[0];
      const introAction = mixer.clipAction(defaultClip);
      introAction.setLoop(THREE.LoopRepeat, Infinity);
      introAction.play();
    }
  ...
  function startIntro() {
    // Ensure the new Mixamo animation plays continuously when loading finishes
    if (gltf.animations && gltf.animations.length > 0) {
      const defaultClip = gltf.animations[0];
      const introAction = mixer.clipAction(defaultClip);
      introAction.setLoop(THREE.LoopRepeat, Infinity);
